...
def visualizer_overview(message):
    ...
    print("MySQL + strings: {0:>8.3f}".format(end - start))
    start = timer()
    v = visualizer.Visualizer()
    end = timer()
    print("visualizer init: {0:>8.3f}".format(end - start))
    start = timer()
    v.build_background()
    end = timer()
    print("background: {0:>8.3f}".format(end - start))
    start = timer()
    v.build_image('images/static/screen.png', 0, 0, visualizer.WIDTH, visualizer.HEIGHT, mode="hard-light")
    end = timer()
    print("hard light: {0:>8.3f}".format(end - start))
    start = timer()
    v.build_image('images/static/border.png', 0, 0, visualizer.WIDTH, visualizer.HEIGHT)
    end = timer()
    print("borders: {0:>8.3f}".format(end - start))
    start = timer()
    ...
    print("text: {0:>8.3f}".format(end - start))
    start = timer()
    v.build_graph(start_x=41, start_y=212, end_x=302, end_y=365, data=graph_data)
    end = timer()
    print("graph: {0:>8.3f}".format(end - start))
    start = timer()
    file = v.finish_image()
    end = timer()
    print("draw: {0:>8.3f}".format(end - start))
    cursor.close()
    conn.close()
